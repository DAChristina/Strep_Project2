...
case_compare <- function(state, observed, pars = NULL) {
  ...
  incidence_modelled <- state[5, , drop = TRUE] # in the example, the 5th row is cases_inc (n_SI_daily), time column deleted
  incidence_observed <- observed$cases
  # invasiveness <- incidence_modelled +
  #   rlnorm(n = length(incidence_modelled), meanlog = -5, sdlog = 0.45) # logN(-5, 0.45) (Dvog et al.???)
  lambda <- incidence_modelled +
    rexp(n = length(incidence_modelled), rate = exp_noise) # based on mcState example
  dpois(x = incidence_observed*, lambda = lambda, log = TRUE) # observed$cases = daily new cases
}
```
...
incidence_compare <- function(state, prev_state, observed, pars = NULL) {
  ...
  lambda <- state[2, , drop = TRUE] + state[3, , drop = TRUE] # change lambda into Asymptomatic + Diseased
  #   rexp(n = length(incidence_modelled), rate = exp_noise) # lambda = n_SI_cumul + rexp(incid, rate), time data reduced
...
begin_t0_value <- c(pars$S_ini, 0, 0, 0, 0, 0)  # Receall S_ini in Pars
...
dt <- 1
filter$run(save_history = TRUE, pars = pars)
...
# For this trial I use the mean number of invasiveness estimate: min = 0.01860361/365) # Invasiveness per carrier per day
time_shift <- mcstate::pmcmc_parameter("time_shift", pars$time_shift)
beta_0 <- mcstate::pmcmc_parameter("beta_0", pars$beta_0)
beta_1 <- mcstate::pmcmc_parameter("beta_1", pars$beta_1)
delta <- mcstate::pmcmc_parameter("delta", pars$delta, min = 0, prior = function(d)
  dgamma(d, shape = 1, scale = 0.2, log = T)
  )
