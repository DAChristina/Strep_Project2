...
# The model below is stochastic, closed system SIR model available in dust
...
Data vs model comparison function
...
filter <- mcstate::particle_filter$new(data = sir_data,
                                       ...
                                       compare = case_compare, # change to incidence_compare or NULL failed
                                       seed = 1L)
...
# Imma create the file in odin based on beta and gamma, a basic SIR model, closed system,
# with number of rows equivalent to the current output, day = nrow(all_date)
# Require:
# sir_stochastic.R
# Strep_SIR_Stochastic_odin.dust.R

true_history <- readRDS("sir_true_history.rds") # weird things happen here
plot_particle_filter(filter$history(), true_history, incidence$day)

# beta and gamma
filter$run(save_history = TRUE, pars = list(dt = dt, beta = 0.2, gamma = 0.1))
plot_particle_filter(filter$history(), true_history, incidence$day)
